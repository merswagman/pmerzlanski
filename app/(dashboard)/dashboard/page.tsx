...
const STATUS_COLORS: Record<ProjectStatus, string> = {
  not_started: 'bg-gray-100 text-gray-700 dark:bg-gray-700/60 dark:text-gray-300',
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        <div>
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-semibold text-gray-900 text-sm dark:text-gray-100">Recent Projects</h2>
            <Link href="/projects" className="text-xs text-indigo-600 hover:underline dark:text-indigo-400">View all</Link>
          </div>
          {recent.length === 0 ? (
            ...
            <div className="space-y-2">
              {recent.map(p => (
                <Link key={p.id} href={`/projects/${p.id}`} className="flex items-center gap-3 rounded-lg bg-white p-3 shadow-sm ring-1 ring-gray-200 hover:ring-indigo-200 transition-all dark:bg-gray-900 dark:ring-gray-700 dark:hover:ring-indigo-500">
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-gray-900 truncate dark:text-gray-100">{p.title}</p>
        ...
        <div>
          <h2 className="font-semibold text-gray-900 text-sm mb-3 dark:text-gray-100">Upcoming Due Dates</h2>
          {upcoming.length === 0 ? (
